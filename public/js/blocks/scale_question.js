...
SirTrevor.Blocks.ScaleQuestion = (function(){
  ...
  return SirTrevor.Block.extend({
    ...
    editorHTML: function() {
      var template = [
        '<div class="ui top right attached label"><i class="resize horizontal circle icon"></i> سوال مقیاسی</div>',
        '<div class="field"><label>متن سوال:</label><div class="st-text-block" contenteditable="true"></div></div>',
        '<div class="hidden divider"></div><div class="field">',
        ...
        '  <div class="field"><label>بیشترین مقدار</label><input type="number" name="max" value="3" min="1"/></div>',
        '<div class="field></div><div class="field"></div></div>',
        '<div class="hidden divider"></div><div class="three fields">',
        ' <div class="field"><label>برچست کمترین</label><input name="minLabel" type="text" placeholder="کمینه"/></div>',
        ' <div class="field"><label>برچسب میانی</label><input name="midLabel" type="text" placeholder="میانه"/></div>',
        ' <div class="field"><label>برچسب بیشترین</label><input name="maxLabel" type="text" placeholder="بیشینه"/></div>',
        '</div>' // /.three.fields
      ].join("\n");
      return template;
    },
    ...
    validations: ['scaleQuestionValidator'],

    // Likert validator
    scaleQuestionValidator: function() {
    ...
    loadData: function(data){
      ...
      this.$("input[name='max']").val(data.max);
      this.$("input[name='minLabel']").val(data.minLabel);
      this.$("input[name='midLabel']").val(data.midLabel);
      this.$("input[name='maxLabel']").val(data.maxLabel);
    },
